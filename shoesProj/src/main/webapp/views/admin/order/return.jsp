...
<script>
	...
	$(function(){
		...
		if(${param.state == null}){
			...
			$("#kind").val($("#${param.kind}${param.state}").attr("kk"))
		}
		...
		$("#modal_close_btn").click(function(){
			...
			$("#statebb").val($("#state").val())
			$("#state").val($("#modalSel").val())
			frm.action = "orderAllReg"
			frm.submit()
       ...
       <div>
	       ...
	       <table> 
				<tr>
					...
					<td>
						<select id="modalSel">
							<option value="완료" <c:if test="${dto.state == '완료' }">selected="selected"</c:if>>처리완료</option>	
						</select>
					</td>
				</tr>
	       </table>
	         <span>
		        * 취소, 완료된 주문은 변경되지 않습니다.
	       </span>
       </div>
        
       <input type="button" id="modal_close_btn1" value="취소">
        <input type="button" id="modal_close_btn" value="확인">
...
<input type="hidden" name="page" id="pageIN" value="${mainData.pageDTO.page }" />
	<input type="hidden" id="kind1" value="${param.kind }"/>
	<input type="hidden" name="tradeid" id="tradeid" value="" />
	<input type="hidden" name="tradeidArr" id="tradeidArr" value="" />
	...
	<div id="table1">
		...
		<div>
			...
			<input type="button" class="btn" id="취소취소완료" kk="취소" value="취소완료" dd="취소완료"/>
			<input type="button" class="btn" id="환불환불요청" kk="환불" value="환불요청" dd="환불요청"/>
			<input type="button" class="btn" id="환불완료" kk="환불" value="환불완료" dd="완료"/>
		</div>
		<div>기간</div>
		<div>
...
<div id="table2" >
	<div id="root">
    	<button type="button" id="modal_opne_btn">일괄처리</button>
	</div>
	<div id="title">
		<div><input type="checkbox" id="checkMain"></div>
		<div>주문일</div>
		<div>거래 번호</div>
		<div>주문자 ID</div>
		<div>주문 금액</div>
	...
	<div id="date">
		<c:forEach items="${mainData.list }" var="dto">
			<div><input class="chk" dd="${dto.tradeid }"  type="checkbox"></div>
