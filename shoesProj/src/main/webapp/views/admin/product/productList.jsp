...
<script>
	...
	$(function(){
		...
		$(".modi").click(function(){
			$("#model").val($(this).attr("mm"))
			frm.action = "productForm"
			frm.submit()
		})
		
		$(".delel").click(function(){
			if(confirm("정말 삭제하시겠습니까?") == 1){
				location.href="productDeReg?model="+$(this).attr("mm") + "&img1=" + $('#delImg').val()			
			}
		})
...
<div id="table2" >
	<div><input type="button" value="상품 등록" id="productinsert" /></div>
	<div id="title">
		<div>상품명</div>
		<div>모델 번호</div>
		<div>브랜드</div>
	...
	<div id="date">
		<c:forEach items="${mainData.list }" var="pp">
			...
			<div>${pp.model }</div>
			<div>${pp.brand }</div>
			<div><script>price("${pp.rel_price }")</script>원</div>
			<div>
				<input class="modi" type="button" value="수정" mm="${pp.model }"> / 
				<input class="delel" type="button" value="삭제" mm="${pp.model }">
				<input type="hidden" value="${pp.img1}" id="delImg">
			</div>
		</c:forEach>
	</div>
	<div class="btnDD">
